    		...
    		foreach ($contributingRolls as $rol => $roldata)
    		{
    			...
    			if (($roldata['rollentyp'] != 'fam') && (array_key_exists($member, $roldata['members'])))
    			{
    						...
    						&& ($roldata['rol_cost_period'] != 1))
    				{
                                        ...
                                        $beitritt = date('Y-m', $time_begin);
                                        $differenz = ($today-$beitritt);
                                        $ergebnis = date("U", $differenz);
                                        $wohnzeit = floor($ergebnis/(3600*24*30));

                                        // WiSe-SoSe Beitritt kurz vor naechstem Semester
                                        if ((strtotime(date('Y').'-09-15') <= $time_begin)
                                           && ($time_begin <= strtotime(date('Y').'-09-30'))
                                           || (strtotime(date('Y').'-03-15') <= $time_begin)
                                           && ($time_begin <= strtotime(date('Y').'-03-31')))
                                        {
                                                $month_begin = date('n', $time_begin);
                                                $month_begin += 1;
                                                $month_end = ($month_begin + 5);
                                        }
                                        
                                        // WiSe Beitritt dieses Jahr und wir rechnen nach 15. Sept ab fuer das naechste Semester
                                        else if ((date('Y') == date('Y', $time_begin)) && (date('n', $time_begin) <= 9)
                                                && (date('n', $time_begin) >= 4) && ((strtotime(date('Y').'-09-15') <= $todayday )
                                                && ($todayday <= strtotime(date('Y').'-12-30'))))
                                        {
                                                $month_begin = 1;
                                                $month_end = 6;
                                        }
                                        
                                        // WiSe Beitritt dieses Jahr Okt - Dez
                                        else if ((date('Y') == date('Y', $time_begin)) && (date('n', $time_begin) <= 12)
                                             && (date('n', $time_begin) >= 10))
                                        {
                                                $month_begin = date('n', $time_begin);
                                                $month_end = 15;
                                        }

                                        // WiSe Beitritt dieses Jahr Jan - Maerz und wohnt hier bis zu 3 Monate
                                        else if ((date('Y') == date('Y', $time_begin)) && ((date('Y', $time_begin)) - (date('Y') -1) == 1) 
                                             && (date('n', $time_begin) <= 3) && (date('n', $time_begin) >= 1) && (4 > $wohnzeit))
                                        {
                                                $month_begin = date('n', $time_begin);
                                                $month_end = 3;
                                        }
                                        
                                        // WiSe Beitritt letztes Jahr Jan - Maerz und wohnt hier mehr als 3 Monate, also seit dem Vorjahr
                                        else if ((date('Y') == date('Y', $time_begin)) && (date('n', $time_begin) <= 3)
                                             && (date('n', $time_begin) >= 1) && (3 < $wohnzeit))
                                        {
                                                $month_begin = 1;
                                                $month_end = 6;
                                        }

                                        // SoSe Beitritt dieses Jahr April - Sept
                                        else if ((date('Y') == date('Y', $time_begin)) && (date('n', $time_begin) <= 9)
                                                && (date('n', $time_begin) >= 4))
                                        {
                                                $month_begin = date('n', $time_begin);
    				...
    				{
    					$members[$member]['FEE_NEW'] += ($roldata['rol_cost'] / 2);
    					if ($roldata['rol_description'] != '')
    					{
    						$members[$member]['CONTRIBUTORY_TEXT_NEW'] .= ' '.$roldata['rol_description'].' ';
    					}
