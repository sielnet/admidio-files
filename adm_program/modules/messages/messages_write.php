...
if ($gValidLogin && $getMsgType === TableMessage::MESSAGE_TYPE_PM && count($arrAllMailRoles) > 0) {
    ...
    // sielnet remove (' .$row['usr_login_name'].')
        $list[] = array($row['usr_id'], $row['last_name'].' '.$row['first_name'].' ', '');
    }
...
if ($getMsgType === TableMessage::MESSAGE_TYPE_PM) {
    ...
    if ($getUserUuid === '') {
        ...
        // sielnet remove ('.$user->getValue('usr_login_name').')
        $sendto = ' ' . $gL10n->get('SYS_TO') . ' ' .$user->getValue('FIRST_NAME').' '.$user->getValue('LAST_NAME').' ';
    }
